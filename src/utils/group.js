 ...
 *     style.
 */
function addGroup({map, name, layers, beforeId}) {
    var beforeLayerId = normalizeBeforeId(map, beforeId);
    const groupId = guid(true);
 ...
 *     the group.
 */
function addLayerToGroup({map, groupId, groupName, layer, beforeId, ignoreBeforeIdCheck = false}) {
    ...
    var groupedLayer = merge(
        {}, 
        layer, 
        {metadata: merge({}, layer.metadata || {}, {group: {id:groupId, name: groupName}})}
    );
